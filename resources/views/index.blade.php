...
<body class="antialiased">
    <div class="h-screen grid place-items-center content-center">
        @if (config('services.google.client_id'))
            <a href="login/google" class="bg-[#50c7c7] text-white px-4 py-4 mt-2 flex rounded-full"">
                <i class="fa-brands fa-google mt-1"></i>
                <span class="inline-block ml-[4px]">Login with Google</span>
            </a>
        @endif
        @if (config('services.facebook.client_id'))
            <a href="login/facebook" class="bg-[#3b5998] text-white px-4 py-4 mt-2 flex rounded-full"">
                <i class="fa-brands fa-facebook mt-1"></i>
                <span class="inline-block ml-[4px]">Login with Facebook</span>
            </a>
        @endif
        @if (config('services.twitter.client_id'))
            <a href="login/twitter" class="bg-[#00acee] text-white px-4 py-4 mt-2 flex rounded-full"">
        ...
        @if (config('services.tiktok.client_id'))
            <a href="login/tiktok" class="bg-[#ff0050] text-white px-4 py-4 mt-2 flex rounded-full"">
